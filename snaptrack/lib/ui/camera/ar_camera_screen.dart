...
class _ARCameraScreenState extends ConsumerState<ARCameraScreen> {
  ...
  Future<void> onPlaneOrPointTapped(
    ...
    if (!_hasPlacedReference && hitTestResults.isNotEmpty) {
      var singleHitTestResult = hitTestResults[0];
      await _addReferenceObject(singleHitTestResult);
    }
  }

  Future<void> _addReferenceObject(ARHitTestResult hitTestResult) async {
    ...
    if (didAddAnchor == true) {
      // Create a cube node for reference
      var newNode = ARNode(
        type: NodeType.localGLTF2,
        uri: null, // We'll use a simple colored cube
  ...
  void _calculateFoodVolume() {
    ...
    // In a real implementation, you would use computer vision
    setState(() {
  ...
  Future<void> _captureWithAR() async {
    if (_estimatedVolume == null && _isARMode) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    // Take screenshot of AR view or use regular camera
